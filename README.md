...
1. Ensure you have the following requirements installed:
   ...
   - PostgreSQL (the latest 13 release).
     - Read a [macOS tutorial](https://www.moncefbelyamani.com/how-to-install-postgresql-on-a-mac-with-homebrew-and-lunchy/)
   ...
   - Flyway 11.0.0 ([homebrew instructions](https://formulae.brew.sh/formula/flyway))

     - After downloading, create a .toml file in the following location: `flyway-11.0.0/conf/flyway.toml` and set the flyway environment variables `environment`, `url`, `user`, `password` and`locations` as
